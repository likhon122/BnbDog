...
import newLogo from "./images/blanco.png"; // Ensure the image path is correct
...
const StartStore: React.FC<StartStoreProps> = ({ onClose }) => {
  ...
  return (
    <div className="start-store-overlay rounded-2xl">
      <div className="start-store-container bg-[#141111]/40 bg-opacity-60 backdrop-filter backdrop-blur-lg">
        <div className="start-store-header">
          <IoArrowBackSharp
            className="back-arrow text-[#fbc688]"
            onClick={onClose}
          />
          <h2 className="store-title text-[#fbc688]">
            Increase the star reward with small TON
          </h2>
        </div>

        <div className="reward-card bg-[#141111] border border-[#FFFFFF3D] bg-opacity-60 backdrop-filter backdrop-blur-lg">
          <div>
            <div className="flex items-center justify-center flex-col gap-3 mb-3">
              <img src={newLogo} alt="logo" className="w-[90px] h-[90px]" />
          ...
          <p className="reward-text text-[#fbc688]">
            2X reward only for 0.2 TON
          </p>
          <button
            className="bg-[#0075d9] px-5 rounded-lg py-2"
